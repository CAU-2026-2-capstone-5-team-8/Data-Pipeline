...
- 신규 `src/data_pipeline/source_registry.py` — `_coerce`/`_build`(타입 힌트 기반
  재귀 변환: `tuple[...]`, `X | None`, 중첩 dataclass 처리)와
  `load_registry_dir(cls, directory) -> dict[str, T]`(디렉터리의 모든 `*.json`을
  로드, 파일명이 반드시 `slug` 필드와 일치해야 함, 중복 slug 거부) 제공.
...
검증: `tests/test_internet_archive.py`(19개), `tests/test_hathitrust.py`(12개) 신규
— 전체 226개 테스트 통과. 실제 네트워크로 `search`/`collect --provider
internet-archive`, `build`(오프라인 재빌드), `enrich-bibliography`의 성공/중복/
...
Phase 0(완료) → ~~Phase 1~~(드롭) → Phase 2(완료) → Phase 3(별도 브랜치에서 진행) →
Phase 4(완료, 이 브랜치)

(주의: 이 브랜치는 PR #26 위에 바로 쌓았고 Phase 3(PR #27)와는 독립적이다. Phase 3
로드맵 내용은 `feat/new-topics` 브랜치의 이 문서에만 반영되어 있다.)
...
- Phase 4(완료): `search`/`collect --provider internet-archive`를 실제 네트워크로
  ...
  확인. 기존 226개 테스트(신규 31개 포함) 전체 통과.
